...
if (!file.exists(paste0(envrmt$path_predictors, "/lai.tif"))) {
  ...
  lai_raster <- readRDS(paste0(envrmt$path_predictors, "/lai.tif"))
...
# detectCores() --> uncomment to detect number of cores on your PC
n_cores <- detectCores() - 2 # leave out two cores (can be changed)
...
# The predictors column probably has to be changed. 
predictors <- training[,c(1:35)]#46 #35 # c(4:38,43,45:46)
#training$Class <- as.factor(training$Class)
response <- training[,"temp"]

# Define parameters for the LLOCV. 
llocv <- CreateSpacetimeFolds(training, spacevar = "tree_id",
                              k = 10, class = "temp")
...
# Run model with FFS using the package 'ranger'
# We should discuss about the arguments passed to this function!!!
if (!file.exists(file.path(envrmt$path_models, "model_with_lidar_predictors_and_wiese.RDS"))) {
  ...
  # A ffs is also possible to run but loading times are very long. Because around 2000 models needs to be calculated. 
  
  saveRDS(model, file.path(envrmt$path_models, "model_with_lidar_predictors_and_wiese.RDS"))
  
} else {
  
  model <- readRDS(file.path(envrmt$path_models, "model_with_lidar_predictors_and_wiese.RDS"))
...
```{r}
# Prediction
predicted = stats::predict(object = model, newdata = testing)

val_df = data.frame(ID = dplyr::pull(testing, "tree_id"),#tree_id # doy_hour_id
                    Observed = dplyr::pull(testing, "temp"),
                    Predicted = predicted)

# Calculating residum
val_df$resi <- val_df$Observed - val_df$Predicted
# Adding station id. Does not need to be added if it was added before. 
#val_df$cst_id <- testing$cst_id

hist(val_df$resi,main = "Residuen observed - predicted",breaks = 40)

# Taking a look which stations are predicted poorly. Same could be done for hours. 
sort(abs(tapply(val_df$resi,val_df$ID,mean)))

# Next three lines need to be executed together otherwise an error occurs.  
plot(val_df$Predicted,val_df$Observed,xlim = c(5,40),ylim = c(5,40),xlab = "Predicted °C",ylab = "Observed °C")
abline(lm(val_df$Observed~val_df$Predicted),col ="red")
abline(coef = c(0,1),col ="blue")

# RMSE
round(sqrt(mean((val_df$Predicted - val_df$Observed)^2, na.rm = TRUE)), 2)

summary(lm(val_df$Observed~val_df$Predicted))
```
