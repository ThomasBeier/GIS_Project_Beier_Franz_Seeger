...
if (!file.exists(paste0(envrmt$path_predictors, "/predictor_stack.RDS"))) {
  ...
  predictors@layers[[63]]@data@names <- "dtm" # --> has to be changed back to 64 if LAI is included
...
if (!file.exists(paste0(envrmt$path_processed, "/df_predictors_response.RDS"))) {
  ...
  df_final <- data.frame(matrix(ncol = 66, nrow =  nrow(trees_climate_merged))) # change ncol to 67 if LAI is included
  colnames(df_final) <- names(predictors)

  for (i in 1:66) { # change to 67 if LAI is included
