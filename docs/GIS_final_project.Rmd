...
predictors <- training[,1:46]#46 #35 # c(4:38,43,45:46)
...
if (!file.exists(file.path(envrmt$path_models, "model_with_lidar_predictors_only.RDS"))) {
  ...
  saveRDS(model, file.path(envrmt$path_models, "model_with_lidar_predictors_only.RDS"))
  
} else {
  
  model <- readRDS(file.path(envrmt$path_models, "model_with_lidar_predictors_only.RDS"))
...
plot(val_df$Predicted,val_df$Observed,xlim = c(5,40),ylim = c(5,40))
