...
result <- model_results(training,testing,df_predictors_wiese_merged,predictor_stack,2,97,c(53,3,76),3,2)

df_full_results <- rbind(df_full_results, result)

result <- model_results(training,testing,df_predictors_wiese_merged,predictor_stack,2,8,c(29,85,5),3,2)
...
```{r}
# Calculate mean RMSE per amount of stations left out
df_full_results_grouped <- group_by(df_full_results, amount_station_out)

df_rmse_gesamt_mean <- summarize(df_full_results_grouped,
                                 rmse_gesamt_mean = round(mean(rmse_left_out), 2),
                                 rmse_gesamt_sd = round(sd(rmse_left_out), 2))

# Different plotting
# Barplot with errorbars
ggplot(df_rmse_gesamt_mean, aes(x = amount_station_out, y = rmse_gesamt_mean)) +
  geom_bar(stat = "identity", fill = "red", alpha = 0.7) + 
  scale_x_discrete(limits = c(1,4,8,12,16)) + 
  labs(x = "Number of stations left out", y = "Mean RMSE") + 
  geom_errorbar(aes(x = amount_station_out,
                    ymin = rmse_gesamt_mean-rmse_gesamt_sd, ymax = rmse_gesamt_mean+rmse_gesamt_sd),
                width = 0.5, size = 0.8)

# Boxplots
df_full_results$amount_station_out <- as.factor(df_full_results$amount_station_out)

ggplot(df_full_results, aes(x = amount_station_out, y = rmse_left_out)) +
  geom_boxplot() +
